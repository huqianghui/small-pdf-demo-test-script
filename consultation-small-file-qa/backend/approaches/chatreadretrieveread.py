...
例如:
来源：
info1.txt: 内容 <https://pdftestsa.blob.core.windows.net/consultation4/info1.txt>
输出: 根据[info1.txt](https://pdftestsa.blob.core.windows.net/consultation4/info1.txt)，内容
不要合并来源，而是单独列出每个来源，例如 [info1.txt][info2.pdf].
不要使用引用，而是始终将来源路径放在()中，例如(https://pdftestsa.blob.core.windows.net/consultation4/info1.txt)(https://pdftestsa.blob.core.windows.net/consultation4/info2.pdf).
对于表格形式的数据，请以HTML表格形式输出，不要使用Markdown表格.
回答内容一定要包括来源，一定要要把来源路径放在()中，例如(https://pdftestsa.blob.core.windows.net/consultation4/info1.txt)(https://pdftestsa.blob.core.windows.net/consultation4/info2.pdf).
...
"""
    ...
    def __init__(self, search_client: SearchClient, chatgpt_deployment: str, gpt_deployment: str, sourcefile_field: str, content_field: str, sourcefile_path_field: str, subscription_key: str, bing_search_endpoint: str):
        ...
        self.sourcefile_path_field = sourcefile_path_field
    ...
    def run(self, history: list[dict], overrides: dict) -> any:
        ...
        r = self.search_client.search(question, 
                                          filter=filter,
        ...
        results = [doc[self.sourcefile_field] + ": " + nonewlines(doc[self.content_field]) + " <" + doc[self.sourcefile_path_field] + ">" for doc in r]
        question=history[-1]["user"]
        ...
        return {"data_points": supporting_facts, "answer": wrap_upped_answer, "thoughts": f"Searched for:<br>{question}<br><br>Prompt:<br>" + prompt.replace('\n', '<br>')}
